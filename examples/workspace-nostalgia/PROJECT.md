...
## Lanes
### 1. Topic sourcing (6x daily, every 4h)
- Adaptive: checks queue depth first; only sources if under 30 queued topics.
- Uses domain-zone exploration: reads `REFERENCE/domain-zones.md`, audits existing topic categories, and constructs zone-targeted search queries for underrepresented zones.
- When sourcing, adds 4-5 new topics per run, spread across at least 2 different zones.
...
## Risks / Unknowns
- Queue could drain faster than sourcing replenishes it if multiple lanes accelerate throughput. (Mitigated: sourcing now runs 6x/day with a threshold of 30.)
- Domain-zone coverage could still skew if web search results are consistently richer for some zones. Monitor the category audit output.
