...
### Goal
Check queue depth and source new topics only when the queue is running low (under 30 queued topics). Uses domain-zone exploration to ensure diverse topic coverage across the full VCE domain.
...
- If queue depth < 30: perform a category audit and source 4-5 new topics via zone-targeted web search
...
- Construct search queries targeted at those zones (e.g., "early 2000s mall culture hot topic nostalgia" instead of a single generic query)
...
- `"blocked"` — research failed (e.g., 403/CAPTCHA), skip until manually unblocked

### Selection rule
Find the first topic in `topic-queue.jsonl` whose `status` is `"queued"` (skip `"done"` and `"blocked"`). Among queued topics, prefer highest `priority` (`high` > `medium` > `low`); break ties by queue order. The selected topic's `id` must NOT already appear in `dossier-index.jsonl`.
...
If web research fails 3 times, update the topic's `status` in `topic-queue.jsonl` from `"queued"` to `"blocked"` and add a `"blockedReason"` field (e.g., `"3 fetch failures — 403/CAPTCHA on source sites"`). Report the blocker and stop. The next run will skip the blocked topic and pick the next queued one.
...
### Goal
Take one "strong" first-pass card and produce a full article-ready dossier with richer sourcing, pull quotes, drama, and post angles.
...
- recent briefs in `REFERENCE/briefs/` (for structural pattern anti-repetition check)

### Source quality gate
Before finalizing source anchors, reject bare domain URLs and prefer contemporaneous/primary sources over retrospective roundups. If the dossier's sources are mostly weak, note this as a caveat in the brief.
...
### QA checks
Brief compliance, anti-slop scan, substance check, argument defensibility, source traceability, brand voice compliance, platform fit. See `REFERENCE/qa-rubric.md` for full details.
