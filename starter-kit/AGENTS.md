...
Goal:
Research [YOUR DOMAIN] topics and produce well-sourced topic cards and writeups.
...
5. Check if the selected topic shares a `parentEvent` with other queued or completed topics. If so, note the related IDs in the card's context section — this helps downstream lanes (especially Lane 4 briefs) identify opportunities for cross-referencing or synthesis.
6. Research the topic using web sources (max 3 solid sources; abandon after 3 failed fetches).
7. Produce a topic card following the template and rubric. Assign a gate assessment per your domain's criteria (e.g., VCE: verdict strong/maybe/weak; News: confidence confirmed/developing/unverified).
8. Save the card to `REFERENCE/cards/<id>-<slug>.md`.
9. Append exactly one JSON line to `REFERENCE/dossier-index.jsonl` with: id, topic/event, gate field value, sources count, card path, timestamp.
10. Update the topic's `status` field in `REFERENCE/topic-queue.jsonl` from `"queued"` to `"done"`.
11. Mark the matching line in `topics/inbox.md` as `[x]`.
12. Optionally append one brief line to `LOG.md`.
13. Return a Discord-friendly summary in 6 bullets or fewer.
...
2. Find the first entry that meets your domain's deep-dive gate (e.g., VCE: `verdict: "strong"`; News: `confidence: "confirmed"` or `"developing"`) and `dossier` is `null` or absent.
3. If no eligible topic exists, report "no topics awaiting deep-dive" and stop.
