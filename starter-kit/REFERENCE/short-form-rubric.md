...
- **Sharper tone.** Short-form can be more direct, more casual, more pointed than the long-form version.
...
Before saving the short-form post, use the `process` tool to execute:
```
...
## QA
Lane 5b short-form posts go through the same QA lane as full drafts. The QA rubric's checks apply — concreteness, anti-slop, voice, endings — with the understanding that short-form has tighter tolerances.
