...
- **Before saving the revision**, run `python3 REFERENCE/pipeline-tools.py slop-check <your-revision-file>`. If the result is `FAIL`, fix every flagged pattern before saving. Do NOT submit a revision that fails the mechanical check — it will just come back from QA again.
